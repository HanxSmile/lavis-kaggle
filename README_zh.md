...
<p>
<br>
# AIGC - A Library for Multi Modal Intelligence
...
- OOTDiffusion (Virtual try-on Model)
  - [x] Train 
  - [x] Validation 
  - [ ] Gradio app
- Cat Viton（Virtual try-on Model）
  - [ ] Train 
  - [ ] Validation 
  - [ ] Gradio app
...
本项目针对语言-视觉相关的研究，聚焦AIGC应用，主要囊括了一些文生图扩散模型的下游应用（如virtual try-on），多模态模型（如图文问答）相关的项目代码。

本项目很大程度上借鉴了LAVIS项目的设计逻辑，实现了AIGC相关多个模型的训练、验证、以及gradio部署，希望将各种复杂的算法及其应用统一到同一个框架中，为相关领域的工程师提供开发与调用的便利。同时也希望相关领域的从业者可以加入到项目的开发中，大家共同努力，共同进步推动AIGC开源项目的发展。
<br><br>
