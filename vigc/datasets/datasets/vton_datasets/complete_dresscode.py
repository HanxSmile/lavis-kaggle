...
from vigc.datasets.datasets.vton_datasets.utils.labelmap import label_map
from vigc.datasets.datasets.vton_datasets.utils.posemap import kpoint_to_heatmap
    ...
    def __init__(
        ...
        if "clip_cloth_features" in self.outputlist:
            self.clip_cloth_features = torch.load(os.path.join(
                self.custum_dataroot / 'data', 'clip_cloth_embeddings', 'dresscode', f'{phase}_last_hidden_state_features.pt'),
                map_location='cpu').detach().requires_grad_(False)

            with open(os.path.join(
                    self.custum_dataroot / 'data', 'clip_cloth_embeddings', 'dresscode', f'{phase}_features_names.pkl'),
                    'rb') as f:
        ...
        if "warped_cloth" in self.outputlist:  # Precomputed warped clothing image
            if self.order == 'unpaired':
                warped_cloth = Image.open(
                    os.path.join(self.custum_dataroot, 'data', 'warped_cloths_unpaired', 'dresscode', category,
                ...
                warped_cloth = Image.open(
                    os.path.join(self.custum_dataroot, 'data', 'warped_cloths', 'dresscode', category,
                                 im_name.replace(".jpg", "") + "_" + c_name))
                warped_cloth = warped_cloth.resize((self.width, self.height))
                warped_cloth = self.transform(warped_cloth)  # [-1,1]
            else:
