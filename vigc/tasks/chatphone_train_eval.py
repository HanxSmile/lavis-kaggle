...
@registry.register_task("chatphone_train_eval")
class ChatPhoneTrainEvalTask(BaseTask):
    ...
    def valid_step(self, model, samples):
        ...
        pred, label, id_, row = response["result"], response["label"], response["id"], response["row"]
        text = response["text"]
        for pred_, label_, uid_, row_, text_ in zip(pred, label, id_, row, text):
            this_res = {
                "pred": float(pred_),
                "label": int(label_),
                "id": int(uid_),
