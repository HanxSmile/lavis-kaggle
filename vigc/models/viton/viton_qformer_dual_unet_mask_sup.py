...
@registry.register_model("viton_qformer_dual_unet_mask_sup")
class VitonQformerMaskSupUnet(Blip2Base):
    """
    Viton Qformer Dual Unet Attention Mask Sup model
    """
    PRETRAINED_MODEL_CONFIG_DICT = {
        "default": "configs/models/viton/viton_qformer_dual_unet_mask_sup.yaml",
    }
    ...
    def register_attn_processors(self, unet):
        ...
        for name in unet.attn_processors.keys():
            ...
            if cross_attention_dim is not None:  # cross attention
                ...
                attn_procs[name] = VitonMaskSupAttnProcessor(
    ...
    def setup_mask(adapters, mask, condition_flag):
        for adapter in adapters:
            if isinstance(adapter, VitonMaskSupAttnProcessor):
    ...
    def calculate_attn_loss(self, vton_attn_scores, garm_attn_scores):
        ...
        return total_loss / len(vton_attn_scores)
    ...
    def forward(self, samples):
        if self.target_image == "viton":
            ...
            loss = garm_loss * 0.5 + vton_loss * 0.5 + attn_loss * 1e-2
