...
from .modules import MelStyleEncoder, ResidualCouplingLayer, Flip, WN, ResBlock1, ResBlock2, LRELU_SLOPE
...
class TextEncoder(nn.Module):
    def __init__(
        ...
        self.text_embedding = nn.Embedding(symbol_nums, hidden_channels)
...
class SynthesizerTrn(nn.Module):
    ...
    def decode(self, codes, text, refer, noise_scale=0.5, speed=1):
        if isinstance(refer, list):
            ges = []
            for _refer in refer:
                ge = self.get_ge(_refer)
                ges.append(ge)
            ge = torch.stack(ges, 0).mean(0)
        else:
            ge = self.get_ge(refer)
